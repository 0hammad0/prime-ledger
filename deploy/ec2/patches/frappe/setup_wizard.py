...
def set_missing_values(task):
	"""Fill blanks from System Settings — never overwrite good wizard values with None."""
	if not (task and task.get("args")):
		return
	...
	# Frappe stages use "timezone"; ERPNext/UI often use "time_zone"
	if args.get("time_zone") and not args.get("timezone"):
		args["timezone"] = args["time_zone"]
	if args.get("timezone") and not args.get("time_zone"):
		args["time_zone"] = args["timezone"]
...
def update_system_settings(args):  # nosemgrep
	# Wizard often drops country/currency from the first slide — never leave these empty.
	country = args.get("country") or frappe.db.get_default("country") or "United States"
	if frappe.db.exists("Country", country):
		args["country"] = country
	else:
		args["country"] = "United States" if frappe.db.exists("Country", "United States") else country

	info = get_country_info(args.get("country")) or {}
	if not args.get("currency"):
		args["currency"] = info.get("currency") or frappe.db.get_default("currency") or "USD"
	if not args.get("timezone") and not args.get("time_zone"):
		tzs = info.get("timezones") or []
		args["timezone"] = (tzs[0] if tzs else None) or "Asia/Kolkata"
	if args.get("time_zone") and not args.get("timezone"):
		args["timezone"] = args.get("time_zone")

	number_format = info.get("number_format") or "#,###.##"
	...
	system_settings.update(
		{
			...
			"time_zone": args.get("timezone") or args.get("time_zone"),
			"currency": args.get("currency"),
			"float_precision": 3,
			"rounding_method": "Banker's Rounding",
			"date_format": frappe.db.get_value("Country", args.get("country"), "date_format")
			or info.get("date_format")
			or "dd-mm-yyyy",
			"time_format": frappe.db.get_value("Country", args.get("country"), "time_format")
			or info.get("time_format")
			or "HH:mm:ss",
...
def sanitize_input(args):
	...
	for key, value in list(args.items()):
		# Skip non-strings (None / int / bool) — is_html/strip_html can crash on None
